...
class Opal_Woocommerce_Extra {
    ...
    public function __construct() {
        ...
        include_once(MEDILAZAR_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-wc-widget-layered-nav.php');
    ...
    public function enqueue_scripts($hook) {
        if ($hook === 'post.php' && get_post_type() === 'product') {
            wp_enqueue_script('otf-woocommerce-admin', MEDILAZAR_CORE_PLUGIN_URL . 'assets/js/woocommerce/admin.js', array('jquery'), MEDILAZAR_CORE_VERSION, true);
        }
    }
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'medilazar-core') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'medilazar-core')); ?>
            <input type="number" min="<?php echo $min; ?>" class="short" name="_deal_quantity" id="_deal_quantity"
                   value="<?php echo esc_attr($quantity) ?>">

            <?php
            if ($quantity > 0) {
                echo '<span class="deal-sold-counts" style="clear:both;display:block;"><strong>' . sprintf(_n('%s product is sold', '%s products are sold', max(1, $sales_counts), 'bicomart-core', 'filaminimal-core', 'medilazar-core', 'striz-core', 'medilazar-core', 'medilazar-core'), $sales_counts) . '</strong></span>';
            }
            ?>
        </p>
    ...
    public function taxonomy_metaboxes() {
        ...
        $cmb_term = new_cmb2_box(array(
            ...
            'title'        => __('Metabox', 'medilazar-core'), // Doesn't output for term boxes
        ...
        $cmb_term->add_field(array(
            ...
            'desc'       => __('Show the banner in product category page or any widgets', 'medilazar-core'),
            'id'         => $prefix . '_banner',
            'type'       => 'file',
            'options'    => array(

...
function osf_get_google_fonts() {
	$content = file_get_contents( MEDILAZAR_CORE_PLUGIN_DIR . 'webfonts.json' );
...
if ( ! function_exists( 'osf_scrape_instagram' ) ) {
	...
	function osf_scrape_instagram( $username, $slice = 9 ) {
		...
		if ( false === ( $instagram = get_transient( 'otf-instagram-media-new-' . sanitize_title_with_dashes( $username ) ) ) ) {
			...
			if ( ! $insta_array ) {
				return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'medilazar-core' ) );
			}
			...
			if ( isset( $insta_array['entry_data']['UserProfile'][0]['userMedia'] ) ) {
				...
				return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'medilazar-core' ) );
			}
			...
			switch ( $type ) {
				...
				case 'old_2':
					foreach ( $images as $image ) {
						...
						$caption = esc_html__( 'Instagram Image', 'medilazar-core' );
...
function time_elapsed_string( $datetime, $full = false ) {
	...
	$string = array(
		'y' => __( 'year', 'medilazar-core' ),
		'm' => __( 'month', 'medilazar-core' ),
		'w' => __( 'week', 'medilazar-core' ),
		'd' => __( 'day', 'medilazar-core' ),
		//        'h' => __('hour', 'medilazar-core'),
		//        'i' => __('minute', 'medilazar-core'),
		//        's' => __('second', 'medilazar-core'),
	);
	...
	return $string ? implode( ', ', $string ) . __( ' left', 'medilazar-core' ) : __( 'just now', 'medilazar-core' );
}
